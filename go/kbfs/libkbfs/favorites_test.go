...
func TestFavoritesListFailsDuringAddAsync(t *testing.T) {
	...
	config.mockKbpki.EXPECT().FavoriteAdd(gomock.Any(), favToAddToKBFolder(fav1)).Return(nil)
	...
	c := make(chan struct{})
	config.mockKbpki.EXPECT().FavoriteList(gomock.Any()).Return(keybase1.FavoritesResult{}, nil)
	config.mockKbpki.EXPECT().FavoriteAdd(gomock.Any(), favToAddToKBFolder(fav1)).
